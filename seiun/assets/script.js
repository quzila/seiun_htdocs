(() => {
  ...
  const init = () => {
    ...
    const heroImages = Array.isArray(window.seiunHeroImages) && window.seiunHeroImages.length
      ? window.seiunHeroImages
      : DEFAULT_HERO_IMAGES;

    if (heroLayers.length) {
      heroLayers[0].style.backgroundImage = `linear-gradient(rgba(0,80,150,0.4),rgba(0,122,255,0.3)), url('${heroImages[0]}')`;
      ...
      if (heroLayers.length > 1) {
        ...
        const fadeToNextImage = () => {
          ...
          heroLayers[nextBgIndex].style.backgroundImage = `linear-gradient(rgba(0,80,150,0.4),rgba(0,122,255,0.3)), url('${heroImages[currentImageIndex]}')`;
