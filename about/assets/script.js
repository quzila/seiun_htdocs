  ...
  const initialize = () => {
    ...
    if (heroBackgrounds.length < 2) {
      if (heroBackgrounds.length === 1) {
        heroBackgrounds[0].style.backgroundImage = `linear-gradient(rgba(0,80,150,0.4),rgba(0,122,255,0.3)), url('${heroImages[0]}')`;
    ...
    heroBackgrounds[currentBgIndex].style.backgroundImage = `linear-gradient(rgba(0,80,150,0.4),rgba(0,122,255,0.3)), url('${heroImages[0]}')`;
    heroBackgrounds[currentBgIndex].classList.add('show');

    const fadeToNextImage = () => {
      currentImageIndex = (currentImageIndex + 1) % heroImages.length;
      heroBackgrounds[nextBgIndex].style.backgroundImage = `linear-gradient(rgba(0,80,150,0.4),rgba(0,122,255,0.3)), url('${heroImages[currentImageIndex]}')`;
